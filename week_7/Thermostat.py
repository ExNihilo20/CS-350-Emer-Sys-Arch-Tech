...
class TemperatureMachine(StateMachine):
    ...
    def processTempIncButton(self):
        ...
        self.setPoint += 1
        screen.updateScreen(f"Set Point: {self.setPoint}")
    ...
    def updateLights(self):
        ...
        if self.current_state == self.heat:
            if temp < self.setPoint:  # If the temp is lower than the set point, the thermostat is heating
                if DEBUG:
                    print("redFade()")
                redFade()
            else:
            ...
            if temp > self.setPoint:  # If the temp is higher than the set point, the thermostat is cooling
                if DEBUG:
                    print("blueFade()")
                blueFade()
            else:
            ...
            # Both lights should be off when the thermostat is off
            redLight.off()
            blueLight.off()
            if(DEBUG):
                print("* LEDs off - system off")
    ...
    def setupSerialOutput(self):
        ...
        temp = floor(self.getFahrenheit()) # current temperature
        setPointFahrenheit = self.setPoint # setPoint in Fahrenheit

        # Determine the state as a string
        state_str = self.current_state.id

        # Build the comma delimited string indicating the current thermostat state
        output = f"{state_str},{temp},{setPointFahrenheit}"
    ...
    def manageMyDisplay(self):
        ...
        while not self.endDisplay:
            ...
            # update lights dynamically every second
            self.updateLights()
            # First line: Date & Time (format to fit 16 chars)
            current_time = datetime.now()
            lcd_line_1 = current_time.strftime("%b %d  %I:%M:%S")

            # Second line: Alternate every 5 seconds
            if altCounter < 5:
                # Show temperature
                temp_f = self.getFahrenheit()
                lcd_line_2 = f"Temp: {temp_f:.1f}F"
            else:
                # Show state and setpoint
                state_str = self.current_state.id.capitalize()
                setpoint_f = self.setPoint
                lcd_line_2 = f"{state_str} {setpoint_f}F"

            # Update LCD display
            screen.lcd.message = f"{lcd_line_1:<16}\n{lcd_line_2:<16}"

            # Alternate every 5 seconds
            altCounter = (altCounter + 1) % 10

            # Update serial every 30 seconds
            if DEBUG:
